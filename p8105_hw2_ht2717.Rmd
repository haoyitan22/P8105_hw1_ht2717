...
clean_tw <- function(sheet_name, wheel_label) {
  ...
  df <- read_excel("data/202509 Trash Wheel Collection Data.xlsx",
...
```{r}
# 1. 读取数据
zori_file <- list.files("data", pattern = "(?i)^zip_zori_.*month.*\\.csv$", full.names = TRUE)
meta_file <- list.files("data", pattern = "(?i)^zip\\s*codes\\.csv$", full.names = TRUE)
...
```
This dataset provides Zillow Observed Rent Index (ZORI) values for ZIP codes across the New York metropolitan area. After reshaping the data from wide to long format and merging with ZIP code metadata, we examined the changes in rent between January 2021 and January 2020. The results show the 10 ZIP codes with the largest rent declines during this period, which were concentrated in Manhattan and Brooklyn. These findings reflect the sharp drop in demand for rentals in high-density urban neighborhoods during the COVID-19 pandemic.
